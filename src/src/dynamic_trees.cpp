...
void DynamicTrees::Link(uint32_t u, uint32_t v) {
  ...
  const auto& [u_element, u_successor] = vertices_[u].Split();
  const auto& [v_element, v_successor] = vertices_[v].Split();
...
void DynamicTrees::Cut(uint32_t u, uint32_t v) {
  ...
  const auto& [uv_element, uv_successor] = uv_it->second->Split();
  const bool uv_vu_in_order =
    uv_element->GetRepresentative() != vu_it->second->GetRepresentative();
  const auto& [vu_element, vu_successor] = vu_it->second->Split();
  edges_.erase(uv_it);
  edges_.erase(vu_it);
